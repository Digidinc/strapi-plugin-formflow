          ...
          <Table colCount={5} rowCount={connections.length}>
            ...
            <Tbody>{connections.map((connection) => {
              ...
              return <Tr key={connection.id}>
                ...
                <Td><Typography>{formatMessage({ id: getTranslation('settings.telegram.credential.stored'), defaultMessage: 'Stored in Strapi' })}</Typography></Td>
...
const ConnectionDialog = ({ editor, onClose, onSaved }: { editor: Editor | null; onClose(): void; onSaved(): Promise<void> }) => {
  ...
  useEffect(() => {
    ...
    setDraft(editor?.kind === 'create' ? { mode: 'stored', token: '' } : resetTelegramCredentialDraft());
  ...
  const credential = (): TelegramCreateCredentialRequest | null => draft.mode === 'keep'
    ? null
    : { type: 'stored', token: draft.token.trim() };
  const valid = name.trim() !== '' && (draft.mode === 'keep' || draft.token.trim() !== '');
  const fingerprint = `${name.trim()}|${draft.mode}|${draft.token.trim()}`;
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if (editor.kind === 'create') await post(API.telegramConnections, buildTelegramCreateRequest(name, draft as Extract<typeof draft, { mode: 'stored' }>));
      else await rawRequest(API.telegramConnection(editor.connection.id), {
        method: 'PATCH',
        body: buildTelegramUpdateRequest(
          name,
          draft.mode === 'keep' ? { mode: 'keep' } : { mode: 'replace', token: draft.token }
        ),
      });
  ...
  return <Dialog.Root open={editor !== null} onOpenChange={(open: boolean) => { if (!open) onClose(); }}><Dialog.Content><Dialog.Header>{formatMessage({ id: getTranslation(editor?.kind === 'edit' ? 'settings.telegram.edit.title' : 'settings.telegram.create.title'), defaultMessage: editor?.kind === 'edit' ? 'Edit Telegram connection' : 'Add Telegram connection' })}</Dialog.Header><Dialog.Body><Flex direction="column" alignItems="stretch" gap={4}>
    ...
    {editor?.kind === 'edit' ? <Field.Root><Field.Label>{formatMessage({ id: getTranslation('settings.telegram.field.action'), defaultMessage: 'Credential action' })}</Field.Label><SingleSelect value={draft.mode} onChange={(value: string) => { setDraft({ mode: value as TelegramCredentialDraft['mode'], token: '' }); invalidate(); }}>
      <SingleSelectOption value="keep">{formatMessage({ id: getTranslation('settings.telegram.action.keep'), defaultMessage: 'Keep current credential' })}</SingleSelectOption>
      <SingleSelectOption value="replace">{formatMessage({ id: getTranslation('settings.telegram.action.replace'), defaultMessage: 'Replace stored token' })}</SingleSelectOption>
    </SingleSelect></Field.Root> : null}
    {draft.mode === 'stored' || draft.mode === 'replace' ? <Field.Root required><Field.Label>{formatMessage({ id: getTranslation('settings.telegram.field.token'), defaultMessage: 'Bot token' })}</Field.Label><TextInput type="password" autoComplete="new-password" value={draft.token} onChange={(event: ChangeEvent<HTMLInputElement>) => { setDraft({ ...draft, token: event.target.value }); invalidate(); }} /><Field.Hint>{formatMessage({ id: getTranslation('settings.telegram.field.token.hint'), defaultMessage: 'Stored in the Strapi database. The existing token is never displayed. Validate the new token before saving.' })}</Field.Hint></Field.Root> : null}
